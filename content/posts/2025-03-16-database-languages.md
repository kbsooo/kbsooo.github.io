...
date: 2025-03-16T23:43:00+09:00
tags: ["Database"]
description: ""
draft: false
---
...
- **도메인 제약 조건 (Domain Constraints):** 모든 속성(attribute)에는 가능한 값의 도메인(예: 정수형, 문자형, 날짜/시간형)이 연관되어야 합니다. 특정 도메인에 속한다고 선언된 속성은 해당 도메인 내의 값만 가질 수 있는 제약 조건으로 작용합니다. 도메인 제약 조건은 가장 기본적인 형태의 무결성 제약 조건입니다. 새로운 데이터 항목이 데이터베이스에 입력될 때마다 시스템에서 쉽게 테스트됩니다.
- **참조 무결성 (Referential Integrity)**: 한 관계(relation)에서 주어진 속성 집합에 나타나는 값이 다른 관계의 특정 속성 집합에도 나타나도록 보장하고자 하는 경우가 있습니다(참조 무결성). 예를 들어, 각 수업에 나열된 학과는 대학에 실제로 존재하는 학과여야 합니다. 더 구체적으로, 수업 레코드의 dept name 값은 학과 관계(department relation)의 일부 레코드의 dept name 속성에 나타나야 합니다. 데이터베이스 수정은 참조 무결성을 위반할 수 있습니다. 참조 무결성 제약 조건이 위반되면, 일반적인 절차는 위반을 초래한 작업을 거부하는 것입니다.
- **권한 부여 (Authorization)**: 데이터베이스의 다양한 데이터 값에 대해 사용자가 허용되는 접근 유형에 따라 차별화하고 싶을 수 있습니다. 이러한 차별화는 **권한 부여(authorization)**로 표현되며, 가장 일반적인 유형은 다음과 같습니다: **읽기 권한(read authorization)**은 데이터를 읽을 수 있지만 수정은 불가능하게 하고, **삽입 권한(insert authorization)**은 새로운 데이터를 삽입할 수 있지만 기존 데이터의 수정은 불가능하게 하며, **수정 권한(update authorization)**은 데이터를 수정할 수 있지만 삭제는 불가능하게 하고, **삭제 권한(delete authorization)**은 데이터 삭제를 허용합니다. 사용자는 이러한 권한 유형을 모두, 일부, 또는 조합으로 부여받을 수 있습니다.
...
- 데이터베이스에 저장된 정보의 검색(retrieval).
- 데이터베이스에 새로운 정보의 삽입(insertion).
- 데이터베이스에서 정보의 삭제(deletion).
- 데이터베이스에 저장된 정보의 수정(modification).

데이터 조작 언어에는 기본적으로 두 가지 유형이 있습니다:

- **절차적 DML (Procedural DMLs)**: 사용자가 필요한 데이터가 무엇인지, 그리고 그 데이터를 어떻게 가져올지 지정해야 합니다.
- **선언적 DML (Declarative DMLs)** (비절차적 DML이라고도 함): 사용자가 필요한 데이터가 무엇인지 지정하면 되며, 그 데이터를 어떻게 가져올지는 지정할 필요가 없습니다.
...
![1.1](/images/database-system-concepts/1-1.png)
