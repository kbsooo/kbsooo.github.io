---
title: "Nested Learning 논문 리뷰: 딥러닝 아키텍처의 환상을 벗기다"
date: 2025-11-09T09:15:00+09:00
tags: ["AI", "Deep Learning", "Nested Learning", "Titans", "Memory Systems", "Optimization", "Paper Review"]
description: "딥러닝의 근본적 한계를 지적하고 중첩 최적화로 새로운 패러다임을 제시하는 Nested Learning 논문의 번역과 심층 분석. 옵티마이저가 연합기억 모듈이라는 혁신적 통찰."
draft: false
translationKey: "nested-learning"
---

## 편집자 노트: 이 논문을 번역하는 이유

> **논문 정보:** Ali Behrouz, Meisam Razaviyayn, Peilin Zhong, Vahab Mirrokni (Google Research). NeurIPS 2025 채택.
> **원문:** [arXiv:2512.24695](https://arxiv.org/abs/2512.24695)

이 논문은 2025년 NeurIPS에서 발표된 Google Research의 연구로, 딥러닝이라는 분야 전체를 관통하는 근본적인 질문을 던진다: **"우리가 만들어 온 딥러닝 아키텍처들은 정말로 '깊은(deep)' 것인가?"**

저자 Ali Behrouz는 [Titans](https://arxiv.org/abs/2501.00663) 모델의 핵심 설계자이기도 하다. Titans는 테스트 타임에 자신의 가중치를 업데이트하는 "학습하는 메모리" 개념으로 시퀀스 모델링 분야에 큰 반향을 일으켰는데, 이 Nested Learning 논문은 그 아이디어를 이론적으로 완성하는 후속 작업이다.

이 논문이 중요한 이유는 세 가지다:

1. **통합적 관점의 제시.** Transformer, Mamba, RWKV, TTT 등 최근 쏟아지는 시퀀스 모델들은 표면적으로 매우 달라 보이지만, Nested Learning은 이들을 모두 "서로 다른 주파수로 업데이트되는 선형 계층들의 조합"이라는 하나의 프레임워크로 통합한다. 이는 아키텍처 비교와 설계에 대한 사고방식 자체를 바꾼다.

2. **옵티마이저에 대한 재해석.** Adam이나 SGD with Momentum 같은 최적화 알고리즘이 사실상 "그래디언트를 압축하는 연합 기억 모듈"이라는 통찰은, 최적화와 아키텍처 설계라는 두 개의 독립적 연구 분야를 하나로 연결한다. 이것은 단순한 수학적 트릭이 아니라, 모델이 어떻게 학습하는지에 대한 근본적인 이해의 전환이다.

3. **자기 수정(Self-Modifying) 시스템의 이론적 기반.** 모델이 자신의 업데이트 규칙을 스스로 학습한다는 개념은, 현재 LLM의 가장 큰 한계인 "배포 후 학습 불가" 문제에 대한 원리적 해법을 제시한다.

이 번역은 원문의 핵심 내용을 한국어로 전달하되, 마지막에 논문의 의의와 현대 아키텍처와의 연결에 대한 별도의 논평을 추가했다.
...
> 이 섹션은 원문에 포함되지 않은 편집자의 분석이다.

### 현대 시퀀스 모델 전쟁의 지도를 다시 그리다

2024년부터 시퀀스 모델링 분야는 전례 없는 다양성의 시대에 접어들었다. Mamba(Gu & Dao, 2023)의 선택적 상태 공간 모델, RWKV의 선형 어텐션 RNN, xLSTM의 지수적 게이팅, TTT-Linear(Sun et al., 2024)의 테스트 타임 학습, 그리고 Titans의 신경 메모리 모듈까지 -- 매 분기마다 "Transformer의 대안"을 표방하는 새로운 아키텍처가 등장한다.

문제는 이 아키텍처들이 각각 고유한 용어와 프레임워크를 사용하기 때문에, 이들 사이의 근본적인 공통점과 차이점을 파악하기 어렵다는 것이다. Nested Learning은 바로 이 혼란에 질서를 부여한다.

NL의 렌즈로 보면:

- **Transformer의 어텐션**은 비매개변수적(non-parametric) 연합 기억으로, 매 토큰마다 업데이트되는 가장 빠른 주파수의 메모리다. 반면 FFN 계층은 사전학습 시에만 업데이트되는 가장 느린 주파수의 메모리다. 즉, 전통적 Transformer는 극단적인 2-level NL 시스템이다.

- **Mamba의 선택적 SSM**은 입력에 따라 상태 전이 행렬을 조절하는 매개변수적 연합 기억이다. 선택적 메커니즘은 NL 관점에서 "어떤 정보를 기억에 압축할지 결정하는 학습된 게이트"에 해당한다.

- **TTT-Linear**은 NL과 가장 직접적으로 연결된다. 히든 스테이트가 선형 모델이고, 업데이트 규칙이 자기지도 학습의 경사하강법 스텝이라는 점에서, TTT는 본질적으로 2-level 중첩 최적화 시스템이다. Nested Learning은 이를 더 깊은 수준으로 확장한다.

- **RWKV의 WKV 메커니즘**은 선형 어텐션의 변형으로, 이 논문의 2.1절에서 보인 것처럼 경사하강법으로 메모리를 최적화하는 연합 기억과 수학적으로 동등하다.

이렇게 보면, 최근의 아키텍처 경쟁은 사실 **"중첩 최적화의 어느 수준에서, 어떤 목적함수로, 어떤 빈도로 메모리를 업데이트하는가"**라는 설계 공간 내의 서로 다른 점을 탐색하는 과정이었던 것이다.

### "옵티마이저가 곧 기억이다"라는 통찰의 파급력

이 논문의 가장 도발적인 주장은 Adam이나 SGD with Momentum 같은 옵티마이저가 "연합 기억 모듈"이라는 것이다. 이것은 단순한 수학적 재해석이 아니라 실용적 함의가 크다.

**첫째, 옵티마이저 설계의 새로운 차원이 열린다.** 기존에 옵티마이저를 설계할 때, 우리는 수렴 속도나 일반화 성능 같은 최적화 이론의 관점에서 접근했다. 하지만 NL은 옵티마이저를 "그래디언트의 역사를 기억하는 신경 모듈"로 재해석함으로써, 메모리 용량, 표현력, 망각 특성 같은 완전히 새로운 설계 축을 열어준다. 논문이 제시하는 Deep Momentum Gradient Descent(DMGD)는 모멘텀을 MLP로 대체하는데, 이는 "더 깊은 기억을 가진 옵티마이저"라는 개념이다.

**둘째, Muon 옵티마이저의 이론적 정당화를 제공한다.** 논문의 식 (24)에서 비선형 출력을 Newton-Schulz 반복으로 설정하고 메모리를 선형 계층으로 두면 Muon 옵티마이저와 동등하다는 것을 보인다. 이는 경험적으로 잘 작동하던 Muon이 왜 효과적인지를 NL 프레임워크가 사후적으로 설명할 수 있음을 의미한다.

**셋째, 아키텍처와 옵티마이저의 공동 설계(co-design)가 가능해진다.** 전통적으로 모델 아키텍처와 옵티마이저는 독립적으로 연구되어 왔다. NL은 이 둘이 동일한 "중첩된 연합 기억 시스템"의 서로 다른 수준이라는 것을 보여줌으로써, 둘을 통합적으로 설계하는 것이 자연스러워진다.

### 자기 수정(Self-Modifying) 시스템: LLM의 근본적 한계를 넘어서

현재 LLM의 가장 큰 한계는 배포 후 학습이 불가능하다는 것이다. 논문이 비유하는 "전향성 기억상실증"은 정확한 진단이다 -- 모델은 사전학습 이전의 지식과 컨텍스트 윈도우 내의 현재 정보만 활용할 수 있으며, 새로운 장기 기억을 형성하지 못한다.

HOPE 아키텍처의 자기 수정 메커니즘은 이 문제에 대한 원리적 해법을 제시한다. 모델이 자신의 업데이트 규칙을 학습한다는 것은, 추론 과정에서도 지속적으로 자신의 파라미터를 조정할 수 있다는 의미다. 이는 Titans가 "테스트 타임 학습"으로 보여준 가능성을 이론적으로 무한한 깊이까지 확장한다.

다만, 현실적 제약도 존재한다. 자기 수정 시스템은 안정성(stability) 문제를 본질적으로 내포한다. 모델이 자신의 업데이트 규칙을 업데이트하고, 그 업데이트 규칙의 업데이트를 업데이트하는... 이 무한 재귀는 수학적으로는 우아하지만, 실제 학습에서 발산하지 않도록 제어하는 것은 상당한 엔지니어링 도전이 될 것이다.

### 실무자를 위한 시사점

**1. 아키텍처 선택에 대한 새로운 사고 틀.** 모델을 선택할 때 "Transformer vs. Mamba vs. RWKV" 같은 이분법적 비교보다, "이 태스크에 필요한 메모리의 시간 스케일은 무엇인가?"라는 질문이 더 유용해진다. 짧은 컨텍스트의 빠른 추론이 필요하면 빠른 주파수의 메모리(어텐션)에 집중하고, 긴 문맥의 지속적 학습이 필요하면 느린 주파수의 메모리(CMS 류)를 추가하는 것이 NL의 처방이다.

**2. 옵티마이저도 아키텍처의 일부다.** AdamW를 기본으로 사용하던 관행을 재고할 시점이다. 특히 지속적 학습이나 few-shot 적응이 중요한 도메인에서는, 옵티마이저의 "기억 용량"을 의식적으로 설계하는 것이 성능 향상의 열쇠가 될 수 있다.

**3. 연속 기억 시스템(CMS)의 실용적 가치.** 기존의 Transformer 블록에 서로 다른 주파수로 업데이트되는 MLP 체인을 추가하는 CMS 개념은, 기존 모델을 크게 변경하지 않고도 장기 기억 능력을 보강할 수 있는 실용적 방법을 제시한다. 이는 Transformer의 FFN을 단순한 지식 저장소가 아니라 "특정 주파수의 기억 모듈"로 재설계하라는 제안이다.

### 이 분야는 어디로 향하는가

Nested Learning은 단순히 새로운 아키텍처를 하나 더 추가한 것이 아니다. 그것은 아키텍처 설계라는 게임의 규칙 자체를 재정의한다. "층을 더 깊게 쌓자"에서 "학습의 수준을 더 깊게 중첩하자"로의 전환은, 지난 10년간 지배적이었던 "scale up" 패러다임에 대한 근본적인 대안을 제시한다.

앞으로 예상되는 방향은 다음과 같다:

- **하이브리드 아키텍처의 이론적 정당화.** 이미 실무에서는 Transformer + Mamba, Attention + SSM 같은 하이브리드 모델이 사용되고 있지만, 왜 어떤 조합이 잘 작동하는지에 대한 이론적 이해는 부족했다. NL은 "서로 다른 주파수의 메모리를 결합하는 것"이라는 통합적 설명을 제공한다.

- **자율적 학습 시스템의 부상.** HOPE와 같은 자기 수정 시스템이 성숙하면, "학습은 학습 시점에만 발생한다"는 현재의 패러다임이 무너질 것이다. 배포된 모델이 사용 과정에서 지속적으로 자신을 개선하는 시스템은 AI 안전성(safety) 관점에서도 새로운 도전을 제기한다.

- **생물학적 학습과의 수렴.** NL이 인간 뇌의 다중 시간 스케일 학습에서 영감을 받았다는 것은 우연이 아니다. 시냅스 가소성, 시스템 통합, 수면 중 기억 재생 등 신경과학의 개념들이 점점 더 공학적으로 구현 가능해지고 있으며, NL은 그 교차점에 서 있다.

이 논문은 우리에게 한 가지 불편한 진실을 일깨운다: 우리가 "딥러닝"이라고 불러온 것의 상당 부분은, 실제로는 그렇게 깊지 않았다. 진정한 깊이는 층의 수가 아니라, 학습의 수준(levels of learning)에 있다.